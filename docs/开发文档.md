...
| 单例名 | 文件 | 职责 | 现状 |
|---|---|---|---|
| `EventBus` | `scripts/event_bus.gd` | 无状态行为信号派发（场景切换、对话请求、手册切换、线索选择等） | ✅ 已扩展至线索/疑点联动所需信号 |
| `DataManager` | `scripts/data_manager.gd` | 运行时数据中心（线索/疑点/好感度/世界Flags）；数据变更后由自身发出信号 | ✅ 已扩展线索/疑点/推理状态管理（Phase 4 Task 4.2/4.3 完成） |
...
- `chapter_id: String`
- `room_id: String`
- `category_path: PackedStringArray`（用于线索栏分层展示）
- `tags: PackedStringArray`（用于搜索与筛选）
...
- `child_clue_ids: PackedStringArray`（记录该线索对应的深入调查子线索 ID 列表）
...
- `get_discovered_clues() -> PackedStringArray`（按 `discover_order`）
- `get_clues_by_category_path(path: PackedStringArray) -> PackedStringArray`
- `get_clues_by_tag(tag: String) -> PackedStringArray`
- `get_parent_clue_id(clue_id: String) -> String`
- `get_child_clue_ids(clue_id: String) -> PackedStringArray`
...
##### 6.3.1 SuspicionData 字段规范（当前实现）

当前 `SuspicionData`（`scripts/logic/suspicion_data.gd`）采用如下字段：

- `id: String`
- `title: String`
- `description: String`
- `chapter_id: String`
- `category_path: PackedStringArray`
- `tags: PackedStringArray`
- `required_clue_ids: PackedStringArray`
- `match_mode: String`（`exact` / `contains`）
- `conclusion_clue_id: String`

##### 6.3.2 DataManager 疑点运行时状态

`DataManager` 现已维护：

- `suspicion_defs: Dictionary[String, SuspicionData]`：疑点静态定义缓存。
- `suspicions: Dictionary[String, Dictionary]`：疑点运行时状态，当前键包括：
  - `discovered: bool`
  - `discover_order: int`
  - `read: bool`
  - `resolved: bool`
...
- `add_suspicion(suspicion_id: String, source_type: String = "dialogue", source_id: String = "") -> bool`
- `has_suspicion(suspicion_id: String) -> bool`
- `get_suspicion_state(suspicion_id: String) -> Dictionary`
- `get_all_suspicions() -> PackedStringArray`
- `is_suspicion_resolved(suspicion_id: String) -> bool`
- `mark_suspicion_read(suspicion_id: String) -> void`
- `resolve_suspicion(suspicion_id: String) -> bool`

##### 6.3.3 推理校验与结论生成

- 推理校验由 `ReasoningSystem.validate()` 负责，当前支持 `exact` / `contains` 两种模式。
- 当前 UI 交互固定为 3 个线索槽位。
- 同一个线索不能在同一疑点的多个槽位中重复选择。
- 推理成功后由 `DataManager.resolve_suspicion()` 标记疑点已解决，并调用 `add_clue(conclusion_clue_id, "reasoning", suspicion_id)` 写入结论线索。
- 推理成功后的结果展示复用线索栏 `interaction` 模式；失败时当前实现不额外提示。

##### 6.3.4 当前阶段的疑点触发来源

- 当前已落地来源：**第一章对话事件**（`.dtl` 内直接调用 `DataManager.add_suspicion()`）。
- 文档原规划中的“场景交互触发”“其他结论推导触发”保留为后续扩展点，尚未在当前代码中展开。

### 7. UI 设计

#### 7.1 线索栏 UI

为满足 Phase 5 Task 5.1 的实现一致性，线索栏采用“固定设计尺寸 + 居中展示 + 双模式复用”的架构。

##### 7.1.1 目标与边界

- 目标分辨率基准：游戏窗口 1920×1080。
- UI 设计尺寸：1280×800（在游戏画面中居中显示）。
- 打开线索栏后，背景游戏画面变暗，但不替代暂停逻辑；暂停状态仍由 `GameManager` 统一管理。
- 本组件只负责展示与用户操作，不承担业务判定；业务数据只读 `DataManager`，行为请求通过既有信号流转。

##### 7.1.2 场景与节点分层

建议采用独立 UI 场景（`scenes/UI/clue_panel.tscn`）+ UI 脚本（`scripts/UI/clue_ui.gd`），核心层级如下：

- `CluePanel`（`CanvasLayer`，PROCESS_MODE_ALWAYS）
    - `Dimmer`（全屏半透明遮罩，打开面板时显示）
    - `PanelRoot`（全屏 `Control`，负责输入拦截和居中布局）
        - `CenterContainer`
            - `PanelFrame`（固定 1280×800）
                - `HeaderBar`（标题“线索手册”+ 关闭按钮）
                - `BodySplit`（左右分栏）
                    - `CatalogPane`（左栏目录树）
                    - `DetailPane`（右栏详情区）
...
- `HeaderBar`：固定高度，承载标题与关闭入口。
- `CatalogPane`：展示分层目录（一级目录/二级目录/线索条目），支持折叠展开。
- `DetailPane`：展示当前选中线索标题与正文内容。
- 左右栏之间保留明确分隔线，视觉上保持“目录区/内容区”强分区。
- 正文区域使用滚动容器，保证长文本可阅读。
...
- `archive` 模式（线索手册）：
    - 显示全量“已发现线索”；
    - 按 `category_path` 分层折叠；
    - 深入调查子线索在归档中挂载到父线索下，不单独放入“深入调查”独立目录；
    - 排序默认按 `线索.txt` 固定顺序，支持切换为“按发现时间”。
    - 可扩展搜索/标签筛选。
- `interaction` 模式（交互详情）：
    - 仅显示当前交互相关线索；
    - 一般线索显示单条；
...
- `selection` 模式（推理选线索）：
    - 左侧仍为 archive 树形目录；
    - 右侧详情面板底部额外出现“选择线索”按钮；
    - 由推理手册请求打开，用于为某个推理槽位回填线索。

##### 7.1.5 通信与数据来源约定

- 目录与详情读取来源：`DataManager`（`get_discovered_clues()`、`get_parent_clue_id()`、`get_child_clue_ids()` 等）。
- 交互详情入口：监听 `EventBus.clue_interaction_details_requested(payload)`。
- 数据刷新入口：监听 `DataManager.clue_updated(clue_id)`，执行 UI 局部刷新。
- 读状态写回：由 UI 在用户点击条目后调用 `DataManager.mark_clue_read(clue_id)`（仅状态标记，不做剧情逻辑）。
- 刷新行为约束：数据刷新后保持当前选中条目，不回跳到首条。

##### 7.1.6 缩放与适配策略

- 面板逻辑尺寸固定为 1280×800。
- 在任意窗口尺寸下保持居中与等比缩放。
- 缩放优先保证可读性：不拉伸变形，不改变左右分栏语义。
- 遮罩层透明度保持“轻度变暗”级别，避免完全遮挡场景信息。

##### 7.1.7 打开/关闭状态约定

- 打开时：显示 `Dimmer` + `PanelFrame`，并接管 UI 输入焦点。
- 关闭时：隐藏面板并释放输入焦点。
- 面板可由“暂停面板入口”或后续指定快捷入口打开；不在本任务中新增额外业务系统。

##### 7.1.8 本阶段交付边界（Task 5.1）

- 本任务只落地“线索栏 UI 骨架 + archive/interaction 模式容器 + 数据绑定入口”。
- 不包含疑点推理 UI（Phase 5 Task 5.2/5.3）。
- 不包含存档 UI（Phase 7）。

##### 7.1.9 联调增量（2026-04-07）

- 已完成测试入口联调：`main_menu` 的“选项”按钮跳转测试场景，便于线索手册快速回归测试。
- 已完成 DataManager 深入调查补齐：父线索解锁深入调查时，自动将其子线索写入已发现集合与发现顺序。
- 已完成 ClueUI 稳定性修复：通过 deferred 刷新与空值防御规避目录刷新重入导致的空引用崩溃。
- 已完成 ClueUI 体验修正：归档中父子层级展示、排序切换、点击后停留当前条目并消除红点。
- 已完成脚本兼容修复：修正嵌套类型集合写法与 UTF-8 编码一致性问题。
- 已完成推理联动扩展：ClueUI 新增 `selection` 模式，支持被推理手册呼起并返回所选线索。

#### 7.2 推理手册 UI（Phase 5 Task 5.2/5.3 实际落地）

当前实现未单独创建 `reasoning_ui.gd`，而是采用：

- `scripts/UI/suspicion_ui.gd`
- `scenes/UI/suspicion_panel.tscn`
- `scripts/UI/clue_ui.gd` 的 `selection` 模式
...
- `V` 键呼出“推理手册”。
- `C` 键呼出“线索手册”。
- 两个手册同一时刻只允许显示一层；呼出另一手册时，当前手册直接关闭并切换。
- 该互斥行为通过 `EventBus.ui_panel_focus_requested(panel_id)` 统一协调。

##### 7.2.2 布局与交互

- 总体设计尺寸与线索栏一致：1280×800，居中显示。
- 左栏：疑点 archive 树，支持搜索、排序、红点、已解决状态展示。
- 右栏上半部分：疑点标题与正文详情。
- 右栏下半部分：3 个线索槽位 + “推理”按钮。
- 点击某个槽位后，打开线索栏 `selection` 模式；玩家可在其中浏览线索详情并点击“选择线索”返回。

##### 7.2.3 当前交付边界

- 当前仅实现 archive 型疑点手册，不提供 interaction 型疑点详情模式。
- 当前推理失败不弹出额外提示。
- 当前第一章疑点与结论资源已完成首批落地，后续章节按同一资源结构继续补齐。
...
| EventBus → ClueUI | 监听信号 | `clue_interaction_details_requested(payload)` |
...
| ClueItem → DataManager | 直接调用单例 | `add_clue()`, `is_deep_unlocked()`, `mark_deep_unlocked()`, `get_child_clue_ids()`, `set_world_flag()/get_world_flag()` |
| ClueItem → EventBus | 触发信号 | `clue_interaction_details_requested(payload)` |
...
| 推理系统 → DataManager | 直接调用单例 | `resolve_suspicion(id)` |
| 推理手册 ↔ 线索手册 | EventBus 信号 | `clue_selection_requested(context)`, `clue_selected_for_reasoning(context, clue_id)`, `ui_panel_focus_requested(panel_id)` |
...
| EventBus | `scripts/event_bus.gd` | 已扩展信号：`scene_change_requested`、`dialogue_requested`、`clue_interaction_details_requested`、`clue_selection_requested`、`clue_selected_for_reasoning`、`ui_panel_focus_requested` |
| DataManager 线索/疑点接口 | `scripts/data_manager.gd` | 已实现 `clue_defs`/`clue_states`/`suspicion_defs`/`suspicions`、Task 3.2 与 Task 4.2/4.3 查询写入接口及首次发现提示 |
| 线索数据资源 | `scripts/objects/interactable/clue_data.gd` | 已实现 ClueData Resource 简化字段与父子线索关系字段（Phase 3 Task 3.1） |
| 疑点数据资源 | `scripts/logic/suspicion_data.gd` | 已实现 SuspicionData Resource；包含 `chapter_id`、`category_path`、`tags`、`required_clue_ids: PackedStringArray`（Phase 4 Task 4.1） |
| 线索交互组件 | `scripts/objects/interactable/clue_item.gd` | 已实现一般线索单条详情与深入线索“首轮父、后续父+子”详情规则（Phase 3 Task 3.3） |
| 全章节线索资源 | `assets/objects/clues/` | 已完成雪墅/梦墅/异墅三章 ClueData 资源落地与深入调查父子映射（Task 3.4 完成） |
| 线索栏 UI | `scripts/UI/clue_ui.gd` + `scenes/UI/clue_panel.tscn` | 已实现 `archive/interaction` 双模式、目录分层折叠、搜索筛选、排序切换与交互详情展示（Phase 5 Task 5.1，含联调修复） |
| 推理校验 | `scripts/logic/reasoning_system.gd` | 已实现疑点线索组合校验与成功判定（Phase 4 Task 4.3） |
| 推理手册 UI | `scripts/UI/suspicion_ui.gd` + `scenes/UI/suspicion_panel.tscn` | 已实现疑点树、红点、搜索排序、3 槽位推理与线索选择联动（Phase 5 Task 5.2/5.3） |
...
### 当前进度标记（截至 2026-05-16）

- Phase 0：Task 0.1 ~ 0.5 已完成。
- Phase 1：Task 1.1 ~ 1.2 已完成。
- Phase 2：Task 2.1 ~ 2.4 已完成。
- Phase 3：Task 3.1 ~ 3.3 已完成。
- Phase 3：Task 3.4 已完成（雪墅/梦墅/异墅全章节映射完成）。
- Phase 4：Task 4.1 ~ 4.4 已完成。
- Phase 5：Task 5.1 ~ 5.3 已完成。

### 开发顺序调整（2026-05-16）

经 User 明确指示，并结合当前已落地代码，本轮文档进度更新为：

1. 已完成 Phase 4（疑点与推理系统）核心数据、触发与校验实现。
2. 已完成 Phase 5 Task 5.1 ~ 5.3（线索手册、推理手册与推理选线索联动）。
3. 后续可继续推进 Phase 5 Task 5.4/5.5、Phase 6，或继续补齐后续章节的疑点与结论资源。

> 说明：该调整仅改变阶段执行顺序，不改变既定架构边界与接口约束（DataManager / EventBus / UI 职责分离保持不变）。
...
| 3.4 | 建立线索数据资产清单，将 `线索.txt` 条目映射为可配置 ClueData；“深入调查→X）”必须补全父子关系映射（父写 `child_clue_ids`、子写 `parent_clue_id`） | `assets/objects/clues/` + 映射文档 | 线索条目可数据驱动维护，深入调查线索关系可被 UI 正确读取 |
...
| Task | 内容 | 涉及文件 | 验收标准 |
|---|---|---|---|
| 4.1 | 定义 `SuspicionData` Resource；当前实际字段为 `id`、`title`、`description`、`chapter_id: String`、`category_path: PackedStringArray`、`tags: PackedStringArray`、`required_clue_ids: PackedStringArray`、`match_mode: String`、`conclusion_clue_id: String` | `scripts/logic/suspicion_data.gd` | 已完成；Resource 可在编辑器创建和配置，且可表达“严格匹配/包含匹配”两类校验 |
...
| 4.4 | 疑点的触发来源：场景交互、对话事件、其他结论推导；当前阶段实际先落地“第一章对话事件触发”，由 `.dtl` 中直接调用 `DataManager.add_suspicion()` 完成 | 集成测试 | 当前已完成第一章对话来源触发；其余来源保留为后续扩展点 |
...
| Task | 内容 | 涉及文件 | 验收标准 |
|---|---|---|---|
| 5.1 | 搭建线索记录与交互详情复用 UI：同组件支持 `archive/interaction` 两种模式；`archive` 模式按 `category_path` 分层折叠并支持搜索筛选与排序切换（默认固定顺序/按发现时间）；`interaction` 模式仅显示当前交互线索（一般线索单条、深入调查线索父+子分层） | `scripts/UI/clue_ui.gd` + `scenes/UI/clue_panel.tscn` | 线索栏全量分层可用；交互详情只显示当前线索组；深入调查父子层级正确；点击条目后保持当前选中 |
| 5.2 | 搭建疑点记录栏 UI；当前标题为“推理手册”，按 `V` 呼出，布局与线索手册一致，左侧为 archive 树形疑点目录，支持搜索、排序、新疑点红点，右侧为疑点详情与推理区 | `scripts/UI/suspicion_ui.gd` + `scenes/UI/suspicion_panel.tscn` | 已完成；疑点状态显示正确，红点与单层 UI 切换逻辑正常 |
| 5.3 | 搭建推理操作界面；当前实际落地为“推理手册右侧下半区 3 槽位 + 推理按钮”，槽位点击后唤起线索手册 `selection` 模式，右侧详情底部出现“选择线索”按钮；成功后弹出结论线索，失败无额外提示，且同一线索不可重复选择 | `scripts/UI/suspicion_ui.gd` + `scenes/UI/suspicion_panel.tscn` + `scripts/UI/clue_ui.gd` + `scenes/UI/clue_panel.tscn` | 已完成；完整推理流程可操作，线索选择联动与去重规则生效 |
