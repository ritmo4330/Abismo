...
> **信号职责分界**：`EventBus` 只承载无状态的行为请求（"去做某事"）。数据变更通知（"某数据已更新"）由 `DataManager` 自身的 signal 发出，不经过 `EventBus`。
...
| EventBus | `scripts/event_bus.gd` | 已有信号：`change_room_requested`、`item_picked_up`、`inventory_changed` |
...
| 模块 | 文件 | 重构内容 |
|---|---|---|
| NPC 对话 | `scripts/characters/npc/npc.gd` | 移除自身重叠/输入检测，改为继承 Interactable 基类，实现被动响应 |
| 门 | `scripts/scenes/door.gd` | 改为继承 Interactable 或保留为自动触发的特例（需评估） |
| game_root | `scripts/game_root.gd` | 场景管理逻辑提取至 SceneManager，流程管理提取至 GameManager |
...
| 1.2 | 评估门的交互方式：保留 body_entered 自动触发（不走 Interactable），或改为交互键触发。确认后重构 `door.gd` | 重构 `door.gd` | 场景切换流程正常 |
...
| 2.3 | 清理 `game_root.gd`，使其仅作为启动入口调用 SceneManager 和 GameManager | 重构 `game_root.gd` | 游戏启动流程正常 |
