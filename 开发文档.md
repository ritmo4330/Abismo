...
- DataManager 维护 `world_flags: Dictionary`，记录已触发的交互（如 `"clue_01_picked": true`）。
- Interactable 在 `_ready()` 时查询 `DataManager.get_world_flag(interact_id)`，若已触发则 `queue_free()` 自毁。
...
线索系统采用“三层模型”，以支持《线索.txt》中大量“多阶段调查、条件显隐、跨章节变体、角色视角限制”的复杂需求。

##### 6.1.1 三层数据模型（强制）

1. **静态定义层（ClueData Resource）**
    - 存放“是什么”：线索文本、分类、可见条件、调查规则、高亮规则。
    - 设计原则：可序列化、可编辑器批量配置、可被 Dialogic/场景/UI 统一读取。
2. **运行时状态层（DataManager.clue_states）**
    - 存放“玩家当前进度”：是否发现、发现顺序、已读状态、调查次数、来源信息。
    - 设计原则：仅存档可变状态，不复制静态文本，避免存档膨胀。
3. **检索索引层（DataManager.clue_indexes）**
    - 存放“如何快速查”：按分类路径、标签、章节、未读、结论线索建立索引。
    - 设计原则：UI 查询尽量不全表扫描，索引可在读档后重建。

##### 6.1.2 ClueData 字段规范（替换旧 Task 3.1 字段）
...
- `chapter_id: String`：章节标识（如 chapter_01）。
- `world_id: String`：世界层（如 snow_house / dream_house / anomaly_house）。
- `room_id: String`：房间标识。
- `category_path: Array[String]`：分层分类路径（用于 UI 折叠树），替代旧 `category`。
- `tags: Array[String]`：搜索与筛选标签。
- `clue_type: String`：`scene_evidence` / `testimony` / `conclusion` / `lore`。
- `investigate_mode: String`：`one_time` / `multi_stage` / `repeatable`，替代旧 `can_reinvestigate`。
- `max_investigate_count: int`：最大调查次数，`-1` 表示不限制。
- `reveal_clue_ids: Array[String]`：本线索发现后可直接揭示的后续线索。
- `variant_group_id: String`：同一观察点跨阶段变体分组（用于 ① / ①' 这类替换）。
- `variant_priority: int`：变体优先级（同组命中多个时按优先级选择）。
- `highlight_policy: String`：`always` / `never` / `conditional`，替代旧 `highlight_visible`。
- `highlight_required_clues: Array[String]`：高亮前置线索条件（替换旧 `prerequisite_clues`）。
- `required_world_flags: Array[String]`：显示/可交互所需 world flag。
- `blocked_world_flags: Array[String]`：被阻断时不可见或不可交互。
- `required_suspicion_ids: Array[String]`：依赖疑点状态。
- `required_roles: Array[String]`：角色视角限制。

##### 6.1.3 DataManager 运行时状态规范

`DataManager` 对线索相关状态至少维护以下结构：

- `clue_defs: Dictionary[String, ClueData]`：静态定义缓存。
- `clue_states: Dictionary[String, Dictionary]`：运行时状态，建议键：
  - `discovered: bool`：是否已发现
  - `discover_order: int`：发现顺序（用于排序）
  - `read: bool`：是否已读，可用于 UI 显示未读标记
  - `investigate_count: int`：调查次数
  - `source_type: String`（scene / dialogue / reasoning）：线索来源类型
  - `source_id: String`：线索来源 ID（如场景对象 ID、对话时间线名、推理结论 ID等）
- `clue_indexes: Dictionary[String, Variant]`：检索索引缓存（按分类、标签、章节、未读、结论）。

##### 6.1.4 查询与写入接口约定

DataManager 的线索接口按“写入入口统一 + 查询接口细分”执行：
...
- `is_clue_visible(clue_id: String) -> bool`
- `is_clue_highlighted(clue_id: String) -> bool`

##### 6.1.5 线索.txt 落地规则

1. “深入调查（隐藏→X）”必须拆分为独立 `clue_id`，通过 `reveal_clue_ids` 或多阶段调查链连接。
2. 同一物件在不同阶段文本变化（如 ① 与 ①'）必须使用 `variant_group_id` 建模，不允许在脚本硬编码 if/else 文本切换。
3. “仅特定角色可进入/可见”的线索，统一由 `required_roles` + 条件判定控制，不在 UI 层做特判。
4. 关键线索不高亮使用 `highlight_policy = "never"`；条件高亮使用 `highlight_policy = "conditional"` + 条件数组。
5. 线索显示排序以 `discover_order` 为准；分类展示以 `category_path` 为准；搜索优先走 `tags` + 标题关键词。
...
| DataManager 自身 → UI / 业务层 | DataManager 自身信号 | `clue_updated`, `suspicion_updated`, `affinity_changed` |
...
| SaveSystem ↔ DataManager | 序列化/反序列化 | `save_to_dict() -> Dictionary`, `load_from_dict(data: Dictionary)`（`clue_states` 存档，`clue_indexes` 读档后重建） |
...
| 模块 | 计划文件 |
|---|---|
| DataManager | `scripts/data_manager.gd` |
...
| 线索数据 | `scripts/objects/interactable/clue_data.gd` |
...
| 0.2 | 创建 `Interactable` 基类，定义 `interact_id`、`is_one_time` 属性和 `interact(player: Player)` 虚方法；包含 `_ready()` 中查询 `DataManager.world_flags` 的自毁逻辑 | `scripts/objects/interactable/interactable.gd` | 基类可被继承，world_flags 自毁可运行 |
...
| Task | 内容 | 涉及文件 | 验收标准 |
|---|---|---|---|
| 3.1 | 重构 `ClueData` Resource 为分层模型字段：基础文本（`id/title/description`）、分类维度（`chapter_id/world_id/room_id/category_path/tags/clue_type`）、调查规则（`investigate_mode/max_investigate_count/reveal_clue_ids`）、显隐规则（`highlight_policy/highlight_required_clues/required_world_flags/blocked_world_flags/required_suspicion_ids/required_roles`）、变体规则（`variant_group_id/variant_priority`） | `scripts/objects/interactable/clue_data.gd` | Resource 可在编辑器创建并完整表达“多阶段调查 + 条件显隐 + 变体替换” |
| 3.2 | 在 DataManager 中实现“三层存储”线索管理：`clue_defs`（静态定义）+ `clue_states`（运行时状态）+ `clue_indexes`（检索索引）；实现接口 `add_clue(...)`、`has_clue()`、`mark_clue_read()`、`get_clues_by_category_path()`、`get_clues_by_tag()`、`is_clue_visible()`、`is_clue_highlighted()` | `scripts/data_manager.gd` | 线索写入、查询、红点、可见性与高亮判定均可用；`clue_updated` 信号正常发出 |
| 3.3 | 基于 Interactable 实现 `ClueItem` 子类，统一走 DataManager 判定与写入；支持三类行为：一次性调查、多阶段调查链（隐藏线索解锁）、条件显隐（含关键线索不高亮） | `scripts/objects/interactable/clue_item.gd` | 测试场景中可验证：普通线索、深入调查线索、变体线索、条件高亮线索 |
| 3.4 | 建立线索数据资产清单（按 `chapter_id/world_id/room_id` 组织），将 `线索.txt` 中条目映射为可配置的 ClueData 资源；定义统一 `clue_id` 命名规范与映射表 | `assets/dialogues/clues/`（或等效资源目录） + 映射文档 | 核心线索可通过资源驱动加载，避免在脚本硬编码文本 |
...
| Task | 内容 | 涉及文件 | 验收标准 |
|---|---|---|---|
| 5.1 | 搭建线索记录栏 UI：按 `category_path` 分层折叠（章节/世界/房间/分类）、分类内按 `discover_order` 排序、结论线索带"结论"标签、新线索红点提示、支持 `tags` 与关键词搜索筛选 | `scripts/UI/clue_ui.gd` + `scenes/UI/clue_panel.tscn` | 线索分层显示正确，搜索筛选与红点逻辑正常 |
...
| 6.3 | 实现对话中的线索和疑点获取：口供类线索 / 新疑点在对话过程中通过 Dialogic 自定义事件调用 `DataManager.add_clue(clue_id, "dialogue", timeline_name)` / `DataManager.add_suspicion()`；必要时触发对应 `world_flags` 以激活后续线索变体 | Dialogic 配置 | 对话中获得的线索和疑点自动记录，后续条件线索可正确解锁 |
...
| 7.2 | 实现读档：从 JSON 反序列化恢复 DataManager 状态，重建 `clue_indexes`，并通知 SceneManager 加载对应场景和玩家位置 | 同上 | 读档后游戏状态完全恢复，线索检索性能正常 |
