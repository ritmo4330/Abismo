...
| `DataManager` | `scripts/data_manager.gd` | 运行时数据中心（线索/疑点/好感度/世界Flags）；数据变更后由自身发出信号 | ❌ 待创建，将接管现 `PlayerState` 的职责 |
| `GameManager` | `scripts/game_manager.gd` | 游戏全局流程管理（暂停使用 `get_tree().paused`） | ❌ 待创建，将接管现 `game_root.gd` 的部分职责 |
| `SceneManager` | `scripts/scenes/scene_manager.gd` | 场景切换与过渡动画 | ⚠️ 现有 `game_root.gd` + `transition.gd` 承担此职责，需重构整合 |
| `DialogueManager` | `scripts/dialogue/dialogue_manager.gd` | 对话编排与路由（监听 EventBus 请求，解析条件后调用 Dialogic） | ❌ 待创建（方案已确定采用） |
...
该迁移在 Phase 2 的重构任务中执行，不影响当前已完成内容。

### 4.3 门与过道混合切换策略（Phase 1 Task 1.2 已定案）
...
#### 自动触发保留清单（视为 Passage，不视为 Door）

- hall.tscn
  - Passage 节点：F2Left、F2Right、ZouLang
  - 对应 Spawn：SpawnFromFloor2Left、SpawnFromFloor2Right、SpawnFromZouLang
- zou_lang.tscn
  - Passage 节点：Hall
  - 对应 Spawn：SpawnFromHall
- floor2.tscn
  - Passage 节点：HallLeft、HallRight
  - 对应 Spawn：SpawnFromHallLeft、SpawnFromHallRight
...
#### 与 SceneManager 的未来整合要求（硬性）

- Passage 与 Door 的切换请求统一走 EventBus（scene_change_requested）。
- Phase 2 引入 SceneManager 后，SceneManager 必须成为唯一切场景执行者。
- 当前过渡期允许兼容旧信号（change_room_requested），但 Task 1.2 新增代码必须优先使用 scene_change_requested。
- 未来清理 game_root 场景切换职责时，不允许出现 Passage 或 Door 直接调用场景树加载 API。
...
| 通信路径 | 方式 | 具体接口 / 信号 |
|---|---|---|
| Interactable → DataManager | 直接调用单例 | `add_clue(id)`, `set_world_flag(id)` |
...
| Dialogic → DataManager | 直接调用单例 | `add_clue(id)`, `add_suspicion(id)`, `change_affinity(npc_id, delta)` |
| 推理系统 → DataManager | 直接调用单例 | `resolve_suspicion(id, conclusion_clue_id)` |
| SaveSystem ↔ DataManager | 序列化/反序列化 | `save_to_dict() -> Dictionary`, `load_from_dict(data: Dictionary)` |
...
| EventBus | `scripts/event_bus.gd` | 已有信号：`change_room_requested`、`scene_change_requested`、`dialogue_requested` |
...
- Phase 1：Task 1.1 已完成；Task 1.2 待执行。
...
| 2.4 | 清理 `game_root.gd`，使其仅作为启动入口调用 SceneManager 和 GameManager | 重构 `game_root.gd` | 游戏启动流程正常 |
...
| Task | 内容 | 涉及文件 | 验收标准 |
|---|---|---|---|
| 3.1 | 定义 `ClueData` Resource，包含字段：`id: String`、`title: String`、`description: String`、`category: String`（发现位置分类）、`is_conclusion: bool`（是否为推理结论）、`can_reinvestigate: bool`、`highlight_visible: bool`、`prerequisite_clues: Array[String]`（高亮前置条件） | `scripts/objects/interactable/clue_data.gd` | Resource 可在编辑器创建和配置 |
| 3.2 | 在 DataManager 中实现线索管理：`add_clue(clue_id)`、`has_clue(clue_id)`、`get_clues_by_category()`、`mark_clue_read(clue_id)`（用于红点提示）。数据变更后发出 `clue_updated` 信号 | `scripts/data_manager.gd` | 线索增删查正常，信号正常发出 |
| 3.3 | 基于 Interactable 实现 `ClueItem` 子类，支持需求文档中定义的线索行为：一次性收集、可深入调查（多次交互获得不同线索）、条件高亮（依赖前置线索）、关键线索不高亮 | `scripts/objects/interactable/clue_item.gd` | 在测试场景中验证各类线索行为 |
...
| Task | 内容 | 涉及文件 | 验收标准 |
|---|---|---|---|
| 4.1 | 定义 `SuspicionData` Resource，包含字段：`id: String`、`title: String`、`description: String`、`required_clue_ids: Array[String]`（解决该疑点所需的正确线索组合）、`conclusion_clue_id: String`（解决后生成的结论线索 ID） | `scripts/logic/suspicion_data.gd` | Resource 可在编辑器创建和配置 |
...
| 4.3 | 实现推理验证逻辑：玩家在疑点界面选取线索组合后，系统校验是否匹配 `required_clue_ids`；匹配成功则调用 `DataManager.resolve_suspicion(id)`，该方法内部：(1) 标记疑点为"已解决"，(2) 调用 `add_clue()` 将结论作为新线索写入（`is_conclusion = true`），(3) 发出信号 | `scripts/logic/reasoning_system.gd` | 正确线索组合得出结论，错误组合给出提示，结论出现在线索栏 |
...
| Task | 内容 | 涉及文件 | 验收标准 |
|---|---|---|---|
| 5.1 | 搭建线索记录栏 UI：按发现位置（`category`）分类折叠、分类内按发现时间排序、结论线索带"结论"标签、新线索红点提示、支持搜索筛选 | `scripts/UI/clue_ui.gd` + `scenes/UI/clue_panel.tscn` | 线索分类显示正确，红点逻辑正常 |
...
| 5.3 | 搭建推理操作界面：从疑点栏点击"推理"按钮 → 展开已知线索列表（含结论）→ 玩家选取线索组合 → 提交验证 → 成功显示结论 / 失败给出提示 | `scripts/UI/reasoning_ui.gd` + `scenes/UI/reasoning_panel.tscn` | 完整推理流程可操作 |
...
| 6.3 | 实现对话中的线索和疑点获取：口供类线索 / 新疑点在对话过程中通过 Dialogic 自定义事件调用 `DataManager.add_clue()` / `DataManager.add_suspicion()` | Dialogic 配置 | 对话中获得的线索和疑点自动记录 |
...
| Task | 内容 | 涉及文件 | 验收标准 |
|---|---|---|---|
| 7.1 | 实现 `SaveSystem`：将 DataManager 全部运行时数据（线索、疑点、好感度、world_flags）序列化为 JSON 写入本地文件 | `scripts/save_system.gd` | 存档文件正确生成 |
| 7.2 | 实现读档：从 JSON 反序列化恢复 DataManager 状态，通知 SceneManager 加载对应场景和玩家位置 | 同上 | 读档后游戏状态完全恢复 |
